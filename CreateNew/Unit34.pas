...
uses Unit29, Unit30, Unit31, Unit32, Unit33, Unit35, Data.Win.ADODB, Unit36, Unit37,
  ...
  Unit47, Unit48, Unit11,  Unit2, Unit62;
...
begin
 if ((Form34.Type_otvet=5) or (Form34.Type_otvet=6) or (Form34.Type_otvet=7)) then  //имеем массив ответов
  begin
   ...
   Close;
   SQL.clear;
   Sql.Add('Insert into Test(N_testa,Nazv,Opis) ');
   Sql.Add('Values( '+ IntToStr(N) + ',' +  QuotedStr(Form34.GlobNazv) + ',' + QuotedStr(Form34.GlobOpis) + ')');
   ExecSQL;
   ...
   begin
     ...
     squery:= squery +  IntToStr(dip1[k]) + ',';
     //Parameters.ParamByName('16').Value := IntToStr(dip1[2]);
     squery:= squery +  IntToStr(dip2[k]) + ',';
     //Parameters.ParamByName('17').Value := res[1];
     squery:= squery +  QuotedStr(res[1]) + ',';
  ...
  // Form36.Edit2.Text:='ID первой ? записи: '+IntToStr(Form34.startID);
     ...
     if  ((Form34.Type_otvet<>5) and (Form34.Type_otvet<>7)) then
       ...
       for i:=2 to Form34.GlobKolvo do
          ...
          Close;
          SQL.clear;
          Sql.Add('Insert into Test_Information(N_testa,question,bal1,bal2,bal3,bal4,bal5) ');
 ...
 repeat
  ...
  with MyModule.ADOQuery2 do
   begin
    Close;
    SQL.clear;
    Sql.Add('Update Test_Information');
    Sql.Add('Set diapazon1=' + IntToStr(dip1[i]) + ' ,diapazon2=' + IntToStr(dip2[i]) + ' ,result=' + #39 + res[i] + #39 + ' Where N_1='+ IntToStr(ID-1));
    ExecSQL;
   end;
   i:=i+1;
   ID:=ID+1;
